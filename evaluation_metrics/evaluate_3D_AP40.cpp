...
enum CLASSES{PEDESTRIAN=0, CAR=1, CYCLIST=2, VAN=3, TRUCK=4, TRAM=5, PERSON_SITTING=6};
...
// First dim : Easy, moderate, hard. Second dim : classes
const double MIN_OVERLAP[3][NUM_CLASS] = {{0.5, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5}, 
...
void saveAndPlotPlots(string dir_name, string AP_dir_name, string file_name,string obj_type,vector<double> vals[],bool is_aos){
  ...
  FILE *fp2 = fopen((AP_dir_name + "/" + file_name + "_AP_metric" + ".txt").c_str(),"w");
  fprintf(fp2, "%s AP: %f %f %f\n", file_name.c_str(), sum[0] / 11 * 100, sum[1] / 11 * 100, sum[2] / 11 * 100);
  fclose(fp2);
  ...
  for (int32_t j=0; j<2; j++) {
    ...
    if (j==0) {
      fprintf(fp,"set term png size 450,315 font \"sans\" 11\n");
      fprintf(fp,"set output \"%s.png\"\n",file_name.c_str());
    } else {
      fprintf(fp,"set term postscript eps enhanced color font \"sans\" 20\n");
...
vector<int32_t> getEvalIndices(const string& result_dir) {
    ...
    if (dir) {
        while (entity = readdir(dir)) {
            ...
            // Check if file extension is ".ini", if so, skip this file
            if (extension == "ini" || extension == "json") {
                continue;
            }
...
bool eval(string gt_dir, string result_dir){
  ...
  string plot_dir       = result_dir + "/plot_AP40";
  string stats_dir      = result_dir + "/stats_AP40";
  string AP_dir         = result_dir + "/AP40";

  // create output directories
  char command[1024];
  sprintf(command,"mkdir \"%s\"",plot_dir.c_str());
  system(command);
  sprintf(command,"mkdir \"%s\"",stats_dir.c_str());
  system(command);
  sprintf(command,"mkdir \"%s\"",AP_dir.c_str());
  system(command);
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      ...
      fclose(fp_det);
      saveAndPlotPlots(plot_dir, AP_dir, CLASS_NAMES[c] + "_detection", CLASS_NAMES[c], precision, 0);
      if(compute_aos){
        saveAndPlotPlots(plot_dir, AP_dir, CLASS_NAMES[c] + "_orientation", CLASS_NAMES[c], aos, 1);
        fclose(fp_ori);
      }
    }
  }
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_ground[c]) {
      ...
      fclose(fp_det);
      saveAndPlotPlots(plot_dir, AP_dir, CLASS_NAMES[c] + "_detection_ground", CLASS_NAMES[c], precision, 0);
    }
  }
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_3d[c]) {
      ...
      fclose(fp_det);
      saveAndPlotPlots(plot_dir, AP_dir, CLASS_NAMES[c] + "_detection_3d", CLASS_NAMES[c], precision, 0);
    }
  }
