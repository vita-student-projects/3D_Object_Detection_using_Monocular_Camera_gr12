...
def decode_detections(dets, info, calibs, cls_mean_size, thresholds):
    '''
    ...
    for i in range(dets.shape[0]):  # batch
        ...
        for j in range(dets.shape[1]):  # max_dets
            ...
            # check for each class if confidence score is lower than allowed to ignore entry
            if cls_id == 0 and thresholds['thresh_pedestrian'] is not None:
                if score < thresholds['thresh_pedestrian']:
                    continue
            elif cls_id == 1 and thresholds['thresh_car'] is not None:
                if score < thresholds['thresh_car']:
                    continue
            elif cls_id == 2 and thresholds['thresh_cyclist'] is not None:
                if score < thresholds['thresh_cyclist']:
                    continue
            elif cls_id == 3 and thresholds['thresh_van'] is not None:
                if score < thresholds['thresh_van']:
                    continue
            elif cls_id == 4 and thresholds['thresh_truck'] is not None:
                if score < thresholds['thresh_truck']:
                    continue
            elif cls_id == 5 and thresholds['thresh_tram'] is not None:
                if score < thresholds['thresh_tram']:
                    continue
            elif cls_id == 6 and thresholds['thresh_person_sitting'] is not None:
                if score < thresholds['thresh_person_sitting']:
                    continue
            else:
                if score < thresholds['default']:
                    continue
...
def euler_to_quaternion(roll, pitch, yaw):
    ...
    Args:
        roll (float): Rotation angle in radians around the X-axis.
        pitch (float): Rotation angle in radians around the Y-axis.
        yaw (float): Rotation angle in radians around the Z-axis.

    Returns:
        tuple: Quaternion angles (w, x, y, z).
    """
    cy = math.cos(yaw * 0.5)
    sy = math.sin(yaw * 0.5)
