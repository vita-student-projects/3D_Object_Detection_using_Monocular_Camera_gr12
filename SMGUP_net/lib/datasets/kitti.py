...
class KITTI(data.Dataset):
    def __init__(self, root_dir, split, cfg):
        ...
        self.max_objs = 50
        self.writelist = cfg['writelist']       # get all classes from the configuration file
        self.num_classes = len(self.writelist)
        self.class_name = self.writelist        # assign the names (list)
        # self.cls2id = {'Pedestrian': 0, 'Car': 1, 'Cyclist': 2, 'Van': 3, 'Truck': 4, 'Tram': 5, 'Person_sitting': 6}
        self.cls2id = {name: idx for idx, name in enumerate(self.class_name)} # assign an index to each class
        ...
        # path configuration
        self.data_dir = os.path.join(root_dir, 'KITTI', 'data')
        self.image_dir = os.path.join(self.data_dir, 'image_2')
        self.depth_dir = os.path.join(self.data_dir, 'depth')       # NOT USED
    ...
    def get_image(self, idx):
        img_file = os.path.join(self.image_dir, f"{idx:06d}.{self.image_format}")
    ...
    def get_label(self, idx):
        ...
        assert os.path.exists(label_file), f"{label_file}"
    ...
    def get_calib(self, idx):
        ...
        assert os.path.exists(calib_file), f"{calib_file}"
    ...
    def __getitem__(self, item):
        ...
        if self.split!='test':
            ...
            object_level = np.zeros((self.max_objs), dtype=np.int64)
            ...
            for i in range(object_num):
                ...
                # if object is easy, moderate or hard
                if objects[i].trucation <=0.5 and objects[i].occlusion<=2 and (objects[i].box2d[3]-objects[i].box2d[1])>=25:
                    mask_2d[i] = 1

                object_level[i] = objects[i].level
            ...
            targets = {'depth': depth,
                   ...
                   'cls_ids': cls_ids,
                   'mask_2d': mask_2d,           # if object is considered
                   'obj_level': object_level,    # easy, moderate, hard (1,2,3)
                   'obj_num': object_num}        # number of objects
        else:
            targets = {}
        # collect return data
        inputs = img
