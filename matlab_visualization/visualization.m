...
%     text(0,120,'Don''t care region','parent',h(1).axes,'color','c','HorizontalAlignment','left','VerticalAlignment','top','FontSize',14,'FontWeight','bold','BackgroundColor','black');
    ...
    % frame number
    text(size(img,2),0,sprintf('%s set frame %d/%d (index %d)',data_set,img_name,nimages-1, img_idx), 'parent', h(1).axes,'color','g','HorizontalAlignment','right','VerticalAlignment','top','FontSize',14,'FontWeight','bold','BackgroundColor','black');

    % usage instructions
    text(size(img,2)/2,size(img,1),sprintf('''→'': Next Image  |  ''←'': Previous Image  |  ''↑'': +100  |  ''↓'': -100 | ''q'': quit'), 'parent', h(2).axes,'color','g','HorizontalAlignment','center','VerticalAlignment','bottom','FontSize',14,'FontWeight','bold', 'BackgroundColor','black');
end
